...
async def init_db():
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute("""
            ...
            ADD COLUMN IF NOT EXISTS monthly_salary REAL DEFAULT 0
        """)
        await conn.execute("""
            ALTER TABLE categories
            ADD COLUMN IF NOT EXISTS bonus_threshold REAL DEFAULT 0
        """)
        await conn.execute("""
            ALTER TABLE categories
...
async def get_worker_fixed_salary(worker_id: int) -> float:
    """Сумма фиксированных ставок по всем категориям работника"""
    async with pool.acquire() as conn:
        result = await conn.fetchval("""
...
async def get_worker_bonus(worker_id: int, year: int, month: int) -> float:
    ...
    async with pool.acquire() as conn:
        ...
        total_bonus = 0.0
        for cat in cat_rows:
            cat_earned = await conn.fetchval("""
                SELECT COALESCE(SUM(wl.total), 0)
                FROM work_log wl
                JOIN price_list pl ON wl.work_code = pl.code
                WHERE wl.worker_id = $1
                  AND pl.category_code = $2
...
async def get_category_settings(code: str) -> dict:
...
async def update_category_bonus_settings(code: str, monthly_salary: float = None,
    ...
    async with pool.acquire() as conn:
        if monthly_salary is not None:
            await conn.execute(
                "UPDATE categories SET monthly_salary = $1 WHERE code = $2",
                monthly_salary, code)
        if bonus_threshold is not None:
            await conn.execute(
                "UPDATE categories SET bonus_threshold = $1 WHERE code = $2",
                bonus_threshold, code)
...
async def get_inactive_workers(days: int = 14) -> list:
    """Работники без записей последние N дней"""
    async with pool.acquire() as conn:
        rows = await conn.fetch("""
            SELECT w.telegram_id, w.name,
        ...
        """, days)
        return [(r['telegram_id'], r['name'], r['last_work_date'], int(r['days_inactive']))
                for r in rows]
