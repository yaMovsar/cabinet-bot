...
from database import (
    ...
    set_manager_rate, remove_manager_rate,
    get_manager_rates, get_all_manager_ids
)

from states import (
    AdminAddCategory, AdminAddWork, AdminAddWorker, AdminAddGhostWorker,
    AdminAssignCategory, AdminRemoveCategory,
    AdminEditPrice, AdminRenameWorker, AdminManagerRate,
...
from utils import format_date, send_long_message, MONTHS_RU
...
# ==================== УПРАВЛЯЮЩИЙ (ставки за работу бригады) ====================

async def _manager_menu_view():
    """Возвращает (текст, InlineKeyboardMarkup) для меню ставок управляющего."""
    workers = dict(await get_all_workers())
    lines = []
    buttons = []
    for mid in await get_all_manager_ids():
        mname = workers.get(mid, f"ID{mid}")
        for work_code, work_name, rate, emoji, cat_name in await get_manager_rates(mid):
            lines.append(f"👔 {mname}: {emoji}{work_name} → {int(rate)}₽/ед.")
            buttons.append([InlineKeyboardButton(
                text=f"🗑 {mname}: {work_name} ({int(rate)}₽)",
                callback_data=f"mgrdel:{mid}:{work_code}")])
    buttons.append([InlineKeyboardButton(text="➕ Добавить ставку", callback_data="mgradd")])
    text = (
        "👔 Ставки управляющего\n\n"
        "Управляющий получает деньги за работу, сделанную ВСЕЙ бригадой: за каждую "
        "единицу указанной работы ему начисляется его ставка (сверх зарплаты самих "
        "работников). Начисления попадают в его баланс и ведомость автоматически.\n\n"
    )
    text += "\n".join(lines) if lines else "Ставок пока нет."
    return text, InlineKeyboardMarkup(inline_keyboard=buttons)


@router.message(F.text == "👔 Управляющий", AdminFilter())
async def manager_menu(message: types.Message, state: FSMContext):
    await state.clear()
    text, markup = await _manager_menu_view()
    await message.answer(text, reply_markup=markup)


@router.callback_query(F.data.startswith("mgrdel:"))
async def mgr_delete(callback: types.CallbackQuery, state: FSMContext):
    _, mid, code = callback.data.split(":", 2)
    await remove_manager_rate(int(mid), code)
    text, markup = await _manager_menu_view()
    await callback.message.edit_text(text, reply_markup=markup)
    await callback.answer("Удалено")


@router.callback_query(F.data == "mgradd")
async def mgr_add_start(callback: types.CallbackQuery, state: FSMContext):
    workers = await get_all_workers()
    if not workers:
        await callback.answer("⚠️ Нет работников", show_alert=True)
        return
    buttons = [[InlineKeyboardButton(text=name, callback_data=f"mgrw:{tid}")]
               for tid, name in workers]
    buttons.append([InlineKeyboardButton(text="❌ Отмена", callback_data="cancel")])
    await callback.message.edit_text(
        "Кто управляющий? Выберите работника:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
    await state.set_state(AdminManagerRate.choosing_worker)
    await callback.answer()


@router.callback_query(F.data.startswith("mgrw:"), AdminManagerRate.choosing_worker)
async def mgr_choose_work(callback: types.CallbackQuery, state: FSMContext):
    tid = int(callback.data.split(":")[1])
    await state.update_data(manager_id=tid)
    items = await get_price_list()
    if not items:
        await callback.message.edit_text("⚠️ Сначала добавьте виды работ.")
        await state.clear()
        await callback.answer()
        return
    buttons = [[InlineKeyboardButton(
        text=f"{emoji}{name} ({int(price)}₽)", callback_data=f"mgrwk:{code}")]
        for code, name, price, price_type, cat_code, cat_name, emoji in items]
    buttons.append([InlineKeyboardButton(text="❌ Отмена", callback_data="cancel")])
    await callback.message.edit_text(
        "За какую работу начислять управляющему?",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
    await state.set_state(AdminManagerRate.choosing_work)
    await callback.answer()


@router.callback_query(F.data.startswith("mgrwk:"), AdminManagerRate.choosing_work)
async def mgr_enter_rate(callback: types.CallbackQuery, state: FSMContext):
    code = callback.data.split(":", 1)[1]
    work = await get_work_by_code(code)
    if not work:
        await callback.answer("⚠️ Работа не найдена", show_alert=True)
        return
    await state.update_data(work_code=code, work_name=work[1])
    await callback.message.edit_text(
        f"Ставка управляющего за 1 ед. «{work[1]}» (₽).\n"
        f"Введите число, напр. 100:")
    await state.set_state(AdminManagerRate.entering_rate)
    await callback.answer()


@router.message(AdminManagerRate.entering_rate)
async def mgr_save_rate(message: types.Message, state: FSMContext):
    try:
        rate = float(message.text.strip().replace(",", "."))
        if rate < 0:
            raise ValueError
    except (ValueError, AttributeError):
        await message.answer("❌ Введите число, напр. 100")
        return
    data = await state.get_data()
    await set_manager_rate(data["manager_id"], data["work_code"], rate)
    await state.clear()
    await message.answer(
        f"✅ Ставка сохранена: {data['work_name']} → {int(rate)}₽/ед.\n\n"
        f"Начисления появятся у управляющего автоматически по записям бригады.",
        reply_markup=get_edit_keyboard())
...
async def admin_entries_start(message: types.Message, state: FSMContext):
    """Выбор работника для просмотра записей"""
    await state.clear()
    workers = await get_all_workers()
    if not workers:
        await message.answer("⚠️ Нет работников.")
...
async def admin_entries_choose_month(callback: types.CallbackQuery, state: FSMContext):
    ...
    buttons = [
        [InlineKeyboardButton(
            ...
            callback_data=f"ae_month:{current_year}:{current_month}"
        )],
        [InlineKeyboardButton(
            text=f"📅 {MONTHS_RU[prev_month]} {prev_year}",
            callback_data=f"ae_month:{prev_year}:{prev_month}"
        )],
        [InlineKeyboardButton(text="🔙 К работникам", callback_data="ae_back_workers")],
        [InlineKeyboardButton(text="❌ Отмена", callback_data="ae_cancel")]
    ]
...
async def admin_entries_show(callback: types.CallbackQuery, state: FSMContext):
    ...
    data = await state.get_data()
    wid = data["worker_id"]
    wname = data["worker_name"]
    
    await state.update_data(year=year, month=month)
    
    entries = await get_worker_entries_by_month(wid, year, month)
    
    if not entries:
        buttons = [
            [InlineKeyboardButton(text="🔙 К месяцам", callback_data="ae_back_months")],
            [InlineKeyboardButton(text="❌ Отмена", callback_data="ae_cancel")]
        ]
        await callback.message.edit_text(
            f"📭 У {wname} нет записей за {MONTHS_RU[month]} {year}",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons)
        )
        await callback.answer()
        return
...
async def admin_entry_chosen(callback: types.CallbackQuery, state: FSMContext):
    ...
    buttons = [
        ...
        [InlineKeyboardButton(text="🔙 К записям", callback_data="ae_act:back")]
    ]
    await callback.message.edit_text(
        f"📦 <b>{entry[1]}</b>\n\n"
        f"👤 {entry[7]}\n"
...
async def admin_entry_action(callback: types.CallbackQuery, state: FSMContext):
    ...
    if action == "edit":
        ...
        buttons = [
            [InlineKeyboardButton(text="✅ Да, удалить!", callback_data="ae_del:yes")],
...
async def admin_entry_new_qty(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ Изменено!\n\n"
        f"📦 {entry[1]} ({entry[7]})\n"
        f"Было: {int(old_qty)} {unit_label} = {int(old_total)} ₽\n"
        f"Стало: {int(new_qty)} {unit_label} = {int(new_total)} ₽",
        reply_markup=get_edit_keyboard()
    )
    await state.clear()
...
@router.callback_query(F.data == "ae_back_workers")
async def admin_entries_back_to_workers(callback: types.CallbackQuery, state: FSMContext):
    """Возврат к списку работников"""
    workers = await get_all_workers()
    buttons = [[InlineKeyboardButton(text=f"👤 {n}", callback_data=f"ae_w:{t}")] for t, n in workers]
    buttons.append([InlineKeyboardButton(text="❌ Отмена", callback_data="ae_cancel")])
...
@router.callback_query(F.data == "ae_back_months")
async def admin_entries_back_to_months(callback: types.CallbackQuery, state: FSMContext):
    """Возврат к выбору месяца"""
    data = await state.get_data()
    wname = data.get("worker_name", "Работник")
...
@router.callback_query(F.data == "ae_cancel")
async def admin_entries_cancel(callback: types.CallbackQuery, state: FSMContext):
...
async def admin_entries_back(callback: types.CallbackQuery, state: FSMContext):
    ...
    await admin_entries_cancel(callback, state)
...
async def edit_category_chosen(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not cat_info:
        ...
        await state.clear()
        return
    settings = await get_category_settings(code)
...
async def edit_category_action(callback: types.CallbackQuery, state: FSMContext):
    ...
    if action == "name":
        ...
        cur = int(settings.get('monthly_salary') or 0)
        await callback.message.edit_text(
            f"💰 Фиксированная месячная ставка\n"
            f"Категория: {data['cat_emoji']} {data['cat_name']}\n"
            f"Текущая: {cur} руб\n\n"
            f"Введите новую сумму (0 — убрать ставку):"
        )
        await state.set_state(AdminEditCategory.entering_salary)
    elif action == "bonus":
        settings = await get_category_settings(data['cat_code'])
        cur_t = int(settings.get('bonus_threshold') or 0)
        cur_a = int(settings.get('bonus_amount') or 0)
        await callback.message.edit_text(
            f"🏆 Система премирования\n"
            f"Категория: {data['cat_emoji']} {data['cat_name']}\n"
            f"Текущая: +{cur_a} руб за каждые {cur_t} руб\n\n"
            f"Введите порог (например 10000), при котором начисляется премия.\n"
            f"0 — убрать премию:"
        )
...
@router.message(AdminEditCategory.entering_salary)
async def edit_category_salary(message: types.Message, state: FSMContext):
    try:
        ...
        return
    data = await state.get_data()
    await update_category_bonus_settings(data["cat_code"], monthly_salary=salary)
    if salary > 0:
...
@router.message(AdminEditCategory.entering_bonus_threshold)
async def edit_category_bonus_threshold(message: types.Message, state: FSMContext):
    try:
    ...
    if threshold == 0:
        ...
        await state.clear()
        return
    await state.update_data(bonus_threshold=threshold)
    await message.answer(
        f"🏆 Порог: {int(threshold)} руб\n\nТеперь введите размер премии за каждый шаг (например 1000):"
    )
    await state.set_state(AdminEditCategory.entering_bonus_amount)


@router.message(AdminEditCategory.entering_bonus_amount)
async def edit_category_bonus_amount(message: types.Message, state: FSMContext):
    try:
...
@router.callback_query(F.data.startswith("inactive_ok:"))
async def inactive_worker_ok(callback: types.CallbackQuery):
    if callback.from_user.id != ADMIN_ID:
        await callback.answer("⛔ Нет доступа", show_alert=True)
        return
    await callback.message.edit_text("✅ Отмечено — работник всё ещё работает.")
    await callback.answer()


@router.callback_query(F.data.startswith("inactive_fire:"))
async def inactive_worker_fire(callback: types.CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID:
        await callback.answer("⛔ Нет доступа", show_alert=True)
        return
    wid = int(callback.data.split(":")[1])
    workers = await get_all_workers()
    wname = next((n for t, n in workers if t == wid), None)
    if not wname:
        await callback.message.edit_text("⚠️ Работник уже удалён.")
        await callback.answer()
        return
    buttons = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Да, удалить", callback_data=f"inactive_confirm:{wid}")],
        [InlineKeyboardButton(text="❌ Нет", callback_data="inactive_cancel")]
    ])
    await callback.message.edit_text(
        f"⚠️ Удалить {wname} и все его данные?",
        reply_markup=buttons
    )
    await callback.answer()


@router.callback_query(F.data.startswith("inactive_confirm:"))
async def inactive_worker_confirm(callback: types.CallbackQuery):
    if callback.from_user.id != ADMIN_ID:
        await callback.answer("⛔ Нет доступа", show_alert=True)
        return
    wid = int(callback.data.split(":")[1])
    workers = await get_all_workers()
    wname = next((n for t, n in workers if t == wid), "Работник")
    try:
        await delete_worker(wid)
        await callback.message.edit_text(f"✅ {wname} удалён из системы.")
    except Exception as e:
        await callback.message.edit_text(f"❌ Ошибка: {e}")
    await callback.answer()


@router.callback_query(F.data == "inactive_cancel")
async def inactive_worker_cancel(callback: types.CallbackQuery):
    await callback.message.edit_text("✅ Отмечено — работник всё ещё работает.")
    await callback.answer()
